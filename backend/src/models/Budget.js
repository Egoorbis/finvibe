...
export const Budget = {
  ...
  async create(budget, userId) {
    ...
    const result = await db.run(
      `INSERT INTO budgets (user_id, category_id, amount, period, start_date, end_date)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`,
      [scopedUserId, category_id, amount, period, start_date, end_date]
    );
  ...
  async update(id, budget, userId) {
    ...
    await db.run(
      ...
      [category_id, amount, period, start_date, end_date, id, scopedUserId]
    );
  ...
  async delete(id, userId) {
    const scopedUserId = resolveUserId(userId);
    return await db.run('DELETE FROM budgets WHERE id = $1 AND user_id = $2', [id, scopedUserId]);
  },

  // Get budget progress (spent vs budget)
  async getProgress(id, userId) {
    const scopedUserId = resolveUserId(userId);
    const budget = await this.getById(id, scopedUserId);
    ...
    `, [scopedUserId, budget.category_id, budget.start_date, budget.end_date]);
  ...
  async getAllProgress(userId, date = new Date().toISOString().split('T')[0]) {
    const scopedUserId = resolveUserId(userId);
    const budgets = await this.getActive(scopedUserId, date);
